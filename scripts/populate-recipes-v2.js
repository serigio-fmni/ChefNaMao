/**
 * OkCheff — Script de População do Banco (v2)
 * Lógica: 1 combinação por vez, busca 10 receitas, salva em 4 idiomas.
 * Roda uma combinação, confirma no terminal, e segue pra próxima.
 */
...
async function alreadyExists(mealId) {
  const { data } = await supabase.from('recipes').select('id').eq('id', `mealdb_${mealId}_en`).single();
  return !!data;
}

// ── Processa uma receita completa (4 idiomas) ────────────────────────────
async function processMeal(meal, diet, mealType, occasion = null) {
  if (await alreadyExists(meal.idMeal)) {
  ...
  await saveRecipe({
    id: `mealdb_${details.idMeal}_en`, name: nameEN, description: descEN, image: details.strMealThumb,
  ...
  for (const lang of LANGUAGES) {
    ...
    await saveRecipe({
      id: `mealdb_${details.idMeal}_${lang}`, name: tName, description: tDesc, image: details.strMealThumb,
      time_minutes: 30, difficulty: 'medio', servings: 4, diet, meal_type: mealType, occasion,
...
// ── Executa UMA combinação específica ────────────────────────────────────
async function runCombination(label, fetchFn, diet, mealType, occasion = null) {
  log(`\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
...
// ══════════════════════════════════════════════════════════════════════════
// LISTA DE COMBINAÇÕES — GELADEIRA (3 tipos × 4 dietas × 10 receitas)
// ══════════════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════════════
// LISTA DE COMBINAÇÕES — INSPIRAÇÃO (8 ocasiões × 3 dietas × 10 receitas)
// ══════════════════════════════════════════════════════════════════════════
const INSPIRACAO_COMBOS = [
  ...
  { label: 'Inspiração > Aniversário > Vegetariana', fetch: () => fetchMealsByCategory('Dessert', 10), diet: 'vegetariana', occasion: 'Aniversário' },
  { label: 'Inspiração > Aniversário > Vegana', fetch: () => fetchMealsByCategory('Dessert', 10), diet: 'vegana', occasion: 'Aniversário' },
  // Natal
  ...
  { label: 'Inspiração > Natal > Vegana', fetch: () => fetchMealsByCategory('Vegan', 10), diet: 'vegana', occasion: 'Natal' },
  // Páscoa
  { label: 'Inspiração > Páscoa > Tradicional', fetch: () => fetchMealsByCategory('Lamb', 10), diet: 'tradicional', occasion: 'Páscoa' },
  { label: 'Inspiração > Páscoa > Vegetariana', fetch: () => fetchMealsByCategory('Vegetarian', 10), diet: 'vegetariana', occasion: 'Páscoa' },
  { label: 'Inspiração > Páscoa > Vegana', fetch: () => fetchMealsByCategory('Vegan', 10), diet: 'vegana', occasion: 'Páscoa' },
  // Churrasco
  ...
  { label: 'Inspiração > Churrasco > Vegana', fetch: () => fetchMealsByCategory('Vegan', 10), diet: 'vegana', occasion: 'Churrasco' },
  // Réveillon
  ...
  { label: 'Inspiração > Réveillon > Vegana', fetch: () => fetchMealsByCategory('Vegan', 10), diet: 'vegana', occasion: 'Réveillon' },
  // Dia dos Pais
  { label: 'Inspiração > Dia dos Pais > Tradicional', fetch: () => fetchMealsByCategory('Beef', 10), diet: 'tradicional', occasion: 'Dia dos Pais' },
  { label: 'Inspiração > Dia dos Pais > Vegetariana', fetch: () => fetchMealsByCategory('Pasta', 10), diet: 'vegetariana', occasion: 'Dia dos Pais' },
  { label: 'Inspiração > Dia dos Pais > Vegana', fetch: () => fetchMealsByCategory('Vegan', 10), diet: 'vegana', occasion: 'Dia dos Pais' },
];

// ── Main ──────────────────────────────────────────────────────────────────
async function main() {
  log('🚀 OkCheff — População do Banco (lógica combinação por combinação)');
  log(`📊 Total de combinações: ${GELADEIRA_COMBOS.length + INSPIRACAO_COMBOS.length}`);

  log('\n📂 ═══ ABA GELADEIRA ═══');
  for (const combo of GELADEIRA_COMBOS) {
    await runCombination(combo.label, combo.fetch, combo.diet, combo.type);
  }
